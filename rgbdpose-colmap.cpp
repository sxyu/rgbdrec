/** Colmap camera pose estimation wrapper;
...
int main(int argc, char** argv) {
    ...
    // **** BEGIN ARG PARSING ****
    string dataPath, logName, colmapExe, matchType, camModel;
    int numThreads;
    bool rerun, noRefineIntrin;
    po::options_description desc("Option arguments");
    po::options_description descPositional(
        "COLMAP wrapper for RGB-D camera pose estimation, with automatic scale "
        "correction\n(c) Alex Yu 2020\n"
    ...
    std::vector<std::pair<std::string, size_t>> sor;
    // Map matched point id to index in {
    std::map<size_t, size_t> pointIdMap;
    ...
    std::cout << ">>> Writing poses.bin\n";
    {
        ...
        for (size_t i = 0; i < numImages; ++i) {
            transformsFs.write(
                reinterpret_cast<char*>(T.col(sor[i].second).data()),
                12 * sizeof(double));
        }
    }
