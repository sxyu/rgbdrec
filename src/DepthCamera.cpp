...
DepthCamera::CameraConfig::CameraConfig()
    : unflip(false),
      auto_exposure(true),
      integr_time(30.0f),
      analog_gain(2.0f) {}
...
bool DepthCamera::nextFrame(bool removeNoise) {
    ...
    if (config.unflip) {
        cv::flip(xyzMapBuf, xyzMapBuf, 1);
        if (hasRGBMap()) cv::flip(rgbMapBuf, rgbMapBuf, 1);
        if (hasIRMap()) cv::flip(irMapBuf, irMapBuf, 1);
        if (hasAmpMap()) cv::flip(ampMapBuf, ampMapBuf, 1);
        if (hasFlagMap()) cv::flip(flagMapBuf, flagMapBuf, 1);
    }
