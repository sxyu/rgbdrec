...
bool CameraIntrin::operator==(const CameraIntrin& other) {
    if (!(fx == other.fx && fy == other.fy && cx == other.cx && cy == other.cy))
        return false;
    if (memcmp(k, other.k, 6)) return false;
    if (memcmp(p, other.p, 2)) return false;
    return true;
}
bool CameraIntrin::operator!=(const CameraIntrin& other) {
    return !(*this == other);
