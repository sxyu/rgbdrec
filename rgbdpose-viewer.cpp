/** RGB-D camera pose estimation/point cloud registration visualizer.
...
std::vector<string> listDir(const std::string& path,
...
int main(int argc, char** argv) {
    ...
    po::options_description descPositional(
        "Camera pose estimation visualization (c) Alex Yu 2020\n\nPositional "
        "arguments");
    ...
    string transformsFile = (rootPath / "poses.bin").string();
    ...
    std::vector<std::string> images = listDir(imagesPath);
    std::vector<std::string> depths = listDir(depthsPath, ".exr");
